...
class TestCropPlanSchedule(FrappeTestCase):
	...
	@patch("f2c.farm_scheduling.doctype.crop_plan_schedule.crop_plan_schedule.resolve_cluster_warehouses_locations")
	def test_get_available_implements_for_cluster_warehouse_fallback_when_locations_sparse(self, mock_resolve):
		mock_resolve.return_value = {
			"cluster": "CLU-TEST",
			"warehouse_list": ["WH-CLUSTER"],
			"location_list": [],
		...
		def fake_get_value(doctype, *args, **kwargs):
			if doctype == "Implement" and isinstance(args[0], dict) and args[0].get("asset") == "AST-1":
				return SimpleNamespace(name="IMP-WH", implement_name="Warehouse Imp", docstatus=0)
			return None
		...
		with patch(
			...
			"f2c.inventory.logistics_transfer_ticket_api.get_assets_for_warehouse",
			return_value={"assets": [{"name": "AST-1"}]},
		), patch(
			"f2c.farm_scheduling.doctype.crop_plan_schedule.crop_plan_schedule.frappe.db.get_value",
		...
		self.assertEqual(len(out), 1)
		self.assertEqual(out[0]["name"], "IMP-WH")
		self.assertEqual(out[0]["availability"], "at_cluster")
	...
	def test_get_available_implements_for_cluster_in_transit_ltt(self, mock_resolve):
		...
		def fake_get_value(doctype, *args, **kwargs):
			if doctype == "Implement" and args and args[0] == "IMP-LTT" and kwargs.get("as_dict"):
				# Production code uses attribute access on as_dict result
